...
export default function ModulesEditor() {
  ...
  async function addModule() {
    ...
    const data = await mutate('post', `/courses/${courseId}/modules`, { titulo: `Módulo ${order}`, orden: order }, 'No se pudo crear el módulo');
  ...
  async function addLesson(module) {
    ...
    const data = await mutate('post', `/modules/${module.id}/lessons`, { titulo: `Nueva lección ${order}`, contenido: '', orden: order }, 'No se pudo crear la lección');
  ...
  async function uploadMaterial(module, lesson, file, nombre) {
    const formData = new FormData(); formData.append('file', file); formData.append('nombre', nombre || file.name);
  ...
  async function finishConfirmation(phrase) {
    ...
    const body = action === 'publish'
      ? { expectedFingerprint: fingerprint, confirmationToken, phrase }
      : { expectedFingerprint: moduleFingerprint(resource.id) };
...
function ConfirmationDialog({ value, busy, onClose, onConfirm }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4"><div className="bg-white rounded-2xl max-w-md w-full p-6"><h2 className="text-xl font-bold text-titi-dark">{value.action === 'publish' ? 'Confirmar publicación' : 'Confirmar despublicación'}</h2><p className="text-sm text-gray-600 mt-2">{value.action === 'publish' ? 'Revisaste la vista previa. Escribí exactamente esta frase:' : 'El backend actual no entrega confirmación firmada para despublicar. Escribí esta frase para confirmar en interfaz:'}</p><code className="block mt-3 bg-titi-cream rounded-xl p-3 text-sm break-all">{expected}</code><input value={phrase} onChange={(event) => setPhrase(event.target.value)} className="titi-input mt-3" placeholder="Pegá la frase exacta" autoFocus /><div className="flex justify-end gap-3 mt-5"><button type="button" onClick={onClose} className="titi-btn-ghost">Cancelar</button><button type="button" onClick={() => onConfirm(phrase)} disabled={busy || phrase !== expected} className="titi-btn-primary">{busy ? 'Procesando…' : 'Confirmar'}</button></div></div></div>;
}
