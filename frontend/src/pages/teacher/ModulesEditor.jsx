...
export default function ModulesEditor() {
  ...
  async function uploadHtml(module, lesson, file, evaluable, intentosMax) {
    ...
    return mutate('post', `/lessons/${lesson.id}/html`, { html, evaluable, intentosMax: evaluable ? Number(intentosMax) : null, expectedFingerprint: lessonFingerprint(module.id, lesson.id) }, 'No se pudo guardar el HTML');
  }
...
function LessonEditor({ lesson, readOnly, busy, onSave, onUpload, onUploadHtml, onDeleteMaterial, onPublish, onArchive, onRestore, onRestoreRevision }) {
  ...
  useEffect(() => { setTitle(lesson.titulo || ''); setContent(lesson.contenido || ''); setVideoUrl(lesson.videoUrl || ''); setHtmlFile(null); setEvaluable(Boolean(lesson.recursoHtml?.evaluable)); setMaxAttempts(lesson.recursoHtml?.intentosMax || 1); }, [lesson]);
  ...
  async function uploadHtml() { if (!htmlFile) return; setStatus(null); const uploaded = await onUploadHtml(htmlFile, evaluable, maxAttempts); if (uploaded) setStatus('HTML guardado como nueva versión'); }
  return <div className="flex flex-col gap-4">
    ...
    {isHtml && <HtmlResource lesson={lesson} readOnly={readOnly} busy={busy} htmlFile={htmlFile} setHtmlFile={setHtmlFile} evaluable={evaluable} setEvaluable={setEvaluable} maxAttempts={maxAttempts} setMaxAttempts={setMaxAttempts} uploadHtml={uploadHtml} />}
