...
export default function HtmlLessonPlayer({ lessonId, onScoreRecorded }) {
  ...
  useEffect(() => {
    let cancelled = false;
    setSrcDoc(null); setAttemptToken(null); setScore(null); setError(null); setStatus('loading');
    async function load() {
      try {
        ...
        setSrcDoc(withAttemptToken(resource.html, token));
        setStatus('ready');
      } catch (err) {
        if (!cancelled) {
  ...
  useEffect(() => {
    async function receiveScore(event) {
      if (!isTitiScoreMessage(event, iframeRef.current?.contentWindow, attemptToken) || status !== 'ready') return;
  ...
  }, [attemptToken, lessonId, onScoreRecorded, status]);
  ...
  return (
    <section className="mb-6">
      ...
      />
      {evaluable && <p className="mt-3 text-xs font-semibold text-gray-500">{status === 'submitting' ? 'Registrando puntaje…' : score ? `Puntaje de práctica: ${score.score}/100. Mejor: ${score.bestScore}/100.` : 'La actividad registra un puntaje de práctica.'}</p>}
